...
const Classes = () => {
  ...
  const fetchClasses = async () => {
    try {
      const response = await axios.get('https://backend-1u3j.onrender.com/api/v1/class/getall');
  ...
  const handleAddClass = async (e) => {
    ...
    if (newClassName.trim() !== '') {
      try {
        const response = await axios.post('https://backend-1u3j.onrender.com/api/v1/class', { grade: newClassName });
